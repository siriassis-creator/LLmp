...
const LoginRegisterView = ({ onLogin, usersDb, onRegister, onUpdateUser }: { onLogin: (u: User) => void, usersDb: User[], onRegister: (u: User) => void, onUpdateUser: (u: User) => void }) => {
      ...
      try {
        ...
        if (liff.isLoggedIn()) {
          ...
          if (existingUser) {
            ...
            // ดึงชื่อ LINE มาเป็นค่าเริ่มต้น แต่ให้สลับไปโหมดสมัครสมาชิก
  ...
  const handleSubmit = () => {
    ...
    if (isLoginMode) {
      ...
      if (user) {
        // ผูกบัญชี LINE กับบัญชีเก่าที่มีอยู่แล้ว
      ...
      // 🚀 โหมดสมัครสมาชิก
      if (lineProfile) {
        // กรณีสมัครผ่าน LINE (ใช้แค่ชื่อจริง)
        if (!formData.name) return setError('กรุณาระบุชื่อ-นามสกุลจริง สำหรับออกใบวุฒิบัตร');
        const newUser: User = { 
          id: Date.now(), 
          username: `line_${lineProfile.userId.substring(0,8)}`, // Auto-generate
          password: `pass_${Date.now()}`, // Auto-generate
          name: formData.name, 
          role: 'user', 
          registerSource: 'line', 
        ...
        // กรณีสมัครผ่านเว็บปกติ (ต้องกรอกครบ)
  ...
  if (isLiffLoading) {
    return <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50"><div className="text-4xl animate-bounce mb-4 text-[#06C755]">💬</div><p className="text-slate-600 font-bold">กำลังตรวจสอบข้อมูล LINE...</p></div>;
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
      <div className="bg-white p-8 rounded-xl shadow-xl w-full max-w-md border border-slate-200">
        ...
        {lineProfile && (
          <div className="bg-[#f0fcf4] border border-[#a2ecc2] p-4 rounded-xl mb-6 flex items-center gap-4 shadow-sm">
            ...
            <div>
              <p className="text-xs text-green-700 font-bold uppercase">เชื่อมต่อ LINE แล้ว</p>
        ...
        <div className="space-y-4">
          {!isLoginMode && (
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1">ชื่อ - นามสกุลจริง <span className="text-red-500">*</span></label>
              <p className="text-xs text-slate-500 mb-2">ข้อมูลนี้จะถูกนำไปใช้สลักลงบนใบประกาศนียบัตร</p>
              <input type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-900" placeholder="นาย สมชาย ตัวอย่าง" />
            </div>
          )}
          
          {/* ซ่อน Username/Password ถ้าเป็นการสมัครผ่าน LINE เพื่อความง่าย */}
          {(!lineProfile || isLoginMode) && (
            <>
              <div><label className="block text-sm font-bold text-slate-700 mb-1">ชื่อผู้ใช้งาน {isLoginMode ? '' : '(Username)'}</label><input type="text" value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} className="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-900" placeholder="Username" /></div>
              <div><label className="block text-sm font-bold text-slate-700 mb-1">รหัสผ่าน {isLoginMode ? '' : '(Password)'}</label><input type="password" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} className="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-900" placeholder="Password" /></div>
            </>
          )}

          <button onClick={handleSubmit} className={`w-full text-white py-3 rounded-lg transition font-bold shadow-md mt-4 ${lineProfile && !isLoginMode ? 'bg-[#06C755] hover:bg-[#05b34c]' : 'bg-blue-900 hover:bg-blue-800'}`}>
            {isLoginMode ? 'เข้าสู่ระบบ' : (lineProfile ? 'ยืนยันและเริ่มเรียนเลย' : 'สมัครสมาชิกใหม่')}
          </button>
        </div>

        <div className="mt-6 text-center text-sm text-slate-500 border-t pt-4">
          {lineProfile && !isLoginMode ? 'มีบัญชีเดิมอยู่แล้ว? ' : (isLoginMode ? 'ยังไม่มีบัญชีผู้ใช้งาน? ' : 'มีบัญชีอยู่แล้ว? ')}
          <button onClick={() => { setIsLoginMode(!isLoginMode); setError(''); }} className="text-blue-600 font-bold hover:underline">
            {lineProfile && !isLoginMode ? 'เข้าสู่ระบบเพื่อผูกบัญชี' : (isLoginMode ? 'สมัครสมาชิก' : 'เข้าสู่ระบบเลย')}
          </button>
        </div>
